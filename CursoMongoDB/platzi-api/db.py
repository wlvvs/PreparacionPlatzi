...
def agregar_curso(json):
    curso = consultar_curso_por_id_proyeccion(json['id_curso'], {'_id': 0, 'nombre': 1})
    return str(db.carreras.update_one({'_id': ObjectId(json['id_carrera'])}, {'$push': {'cursos': curso}}).modified_count)
...
def actualizar_curso(curso):
    ...
    return db.cursos.update_one({'_id': ObjectId(curso['_id'])}, {'$set': {'nombre': curso['nombre'], 'descripcion': curso['descripcion'], 'clases': curso['clases']}}).modified_count
